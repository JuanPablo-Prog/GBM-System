...
import {
  ...
  Wrench, BarChart3, Monitor, LogOut, Settings, Database, UserCircle
} from 'lucide-react'
...
export default function Sidebar({ currentView, onNavigate }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-60 bg-black/40 backdrop-blur-md border-r border-white/10 flex flex-col z-40 select-none">
      {/* Brand */}
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 bg-amber-DEFAULT rounded-lg flex items-center justify-center flex-shrink-0">
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm
                          ...
                          ${active
                            ? 'bg-amber-muted text-amber-text border border-amber-DEFAULT/20'
                            : 'text-slate-300 hover:text-slate-100 hover:bg-white/10'
                          }`}
            >
              <Icon size={16} className={active ? 'text-amber-DEFAULT' : 'text-slate-400 group-hover:text-slate-200'} />
              <span className={active ? 'font-medium' : ''}>{label}</span>
              {active && (
                <motion.div
                  layoutId="sidebar-pill"
                  className="absolute right-2 w-1 h-1 bg-amber-DEFAULT rounded-full"
                />
              )}
            </button>
          )
      ...
      <div className="px-3 py-3 border-t border-white/10 space-y-1">
        ...
        <button
          onClick={signOut}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm
                     text-slate-400 hover:text-rose-300 hover:bg-rose-500/10 transition-all duration-150"
        >
          <LogOut size={16} />
          Cerrar sesión
        </button>
